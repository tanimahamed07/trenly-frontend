...
import Image from 'next/image'; // ১. Image কম্পোনেন্ট ইম্পোর্ট করুন
...
const HeroSection = () => {
    ...
    return (
        <section className="relative w-full h-[65vh] flex items-center bg-base-100 overflow-hidden border-b border-base-200">
                ...
                <div className="grid lg:grid-cols-12 gap-8 items-center">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <p className="text-base text-neutral/70 max-w-lg leading-relaxed">
                            Discover premium <span className="text-secondary font-bold">Electronics</span> and 
                    ...
                    <div className="lg:col-span-5 flex flex-col items-center gap-6">
                        {/* Main Image Box */}
                        <div className="relative w-full h-[350px] bg-base-200 rounded-[3rem] p-4 border border-base-300 shadow-xl overflow-hidden">
                            <Image 
                                src={activeProduct.image} 
                                alt={activeProduct.title}
                                priority
                                fill // ২. কন্টেইনার অনুযায়ী সাইজ নিতে fill ব্যবহার করুন
                            ...
                            <div className="absolute inset-x-6 bottom-6 bg-base-100/90 dark:bg-base-200/90 backdrop-blur-md p-5 rounded-2xl border border-primary/20 shadow-2xl flex items-center justify-between">
                                <div>
                                    ...
                                    <h4 className="text-lg font-black text-secondary truncate max-w-[150px]">{activeProduct.title}</h4>
                                </div>
                                <div className="text-right">
                                    <p className="text-2xl font-black text-primary leading-none">${activeProduct.price}</p>
                                    <p className="text-[10px] font-bold line-through opacity-40">${activeProduct.originalPrice}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                            {products.map((product) => (
                                <button 
                                    ...
                                    className={`relative w-16 h-16 rounded-2xl overflow-hidden border-2 transition-all ${activeProduct.id === product.id ? "border-primary shadow-lg" : "border-base-300"}`}
                                >
                                    <Image 
                                        src={product.image} 
                                        alt={product.title} 
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 animate-bounce flex flex-col items-center gap-2 text-neutral/40">
