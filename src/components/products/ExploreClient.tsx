    ...
    <div className="bg-base-100 min-h-screen pb-20 text-neutral">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-12">
          {/* Sidebar - Filters */}
          <aside
            className={`fixed inset-y-0 right-0 z-[100] w-[320px] bg-base-100 border-l border-base-300 p-8 shadow-2xl transition-transform duration-500 md:relative md:inset-auto md:z-0 md:w-72 lg:w-80 md:p-0 md:shadow-none md:translate-x-0 md:border-none ${showMobileFilters ? "translate-x-0" : "translate-x-full"}`}
          >
            <div className="sticky top-28 space-y-10">
              <div className="flex items-center justify-between md:hidden">
                ...
                <button
                  onClick={() => setShowMobileFilters(false)}
                  className="p-3 bg-base-200 text-secondary rounded-2xl"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="group space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary/40">
                  Keyword
                </label>
              ...
              <div className="space-y-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateURL({
                      priceMax:
                        Number(e.target.value) < 10000 ? e.target.value : null,
                    })
                  }
          ...
          <main className="flex-1 min-w-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {loading ? (
                Array.from({ length: LIMIT }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-base-200 animate-pulse h-[420px] rounded-[2.5rem] border border-base-300"
                  />
                ))
              ) : products.length > 0 ? (
                ...
                <div className="col-span-full py-32 flex flex-col items-center bg-base-200/30 rounded-[3rem] border-2 border-dashed border-base-300">
                  ...
                  <button
                    onClick={resetFilters}
                    className="mt-4 text-primary font-bold hover:underline underline-offset-8"
                  >
                    Clear filters
                  </button>
                </div>
            ...
            {!loading && totalPages > 1 && (
              <div className="mt-20 flex flex-wrap justify-center items-center gap-3 px-2">
                {/* Previous Button */}
                <button
                  disabled={urlPage === 1}
                ...
                {/* Page Numbers - flex-wrap added for mobile responsiveness */}
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                    (p) => (
                      <button
                        ...
                        className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl font-black text-[10px] md:text-xs transition-all border ${
                          urlPage === p
                            ? "bg-secondary text-base-100 border-secondary shadow-xl shadow-secondary/20 scale-110"
                            : "bg-base-100 border-base-300 hover:border-primary text-secondary"
                        }`}
                      >
                        {p}
                      </button>
