...
import { Trash2, ShieldCheck, User as UserIcon, Loader2, Search } from "lucide-react";
  ...
  const handleToggleRole = (userId: string, currentRole: string) => {
    ...
    toast(`Change User Role?`, {
      ...
      action: {
        ...
        onClick: async () => {
          try {
            const res = await UserService.updateUserRole(userId, newRole, token);
            if (res.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-secondary uppercase tracking-tight">Manage Users</h2>
      ...
      <div className="bg-base-100 border border-base-300 rounded-[2rem] overflow-hidden shadow-sm">
        {loading ? (
          <div className="h-64 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-neutral/30">Syncing database...</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="table table-lg w-full">
              <thead className="bg-base-200/50 border-b border-base-300">
                <tr>
                  <th className="text-[10px] font-black uppercase tracking-widest text-neutral/50">User Details</th>
                  <th className="text-[10px] font-black uppercase tracking-widest text-neutral/50">Access Level</th>
                  <th className="text-[10px] font-black uppercase tracking-widest text-neutral/50">Joined At</th>
                  <th className="text-[10px] font-black uppercase tracking-widest text-neutral/50 text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user._id} className="hover:bg-base-200/30 transition-colors border-b border-base-200 last:border-0">
                    <td>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-black text-secondary">{user.name}</div>
                          <div className="text-[10px] font-bold text-neutral/40 uppercase tracking-tight">{user.email}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                      <span className={`badge badge-sm font-black uppercase tracking-tighter py-3 px-4 rounded-xl border-none ${
                        user.role === 'admin' 
                    ...
                    <td className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        >
                          {user.role === 'admin' ? <UserIcon size={18} /> : <ShieldCheck size={18} />}
                        </button>
                        <button 
            ...
            {filteredUsers.length === 0 && (
              <div className="p-20 text-center">
                ...
                <p className="font-black text-neutral/40 uppercase tracking-widest text-xs">No matching users found</p>
              </div>
            )}
          </div>
        )}
